...
def lines(messages, self_name):
    for contacts in messages:
        ...
        for line in messages[contacts]:
            yield (thread_name, canonicalize(line['contact']),
                   line['timestamp'], line['source'], line['protocol'],
                   line['nick'], line['message'], gender, friendship, age_group)
...
def main(facebook_path, trillian_path, digsby_path, pidgin_path, whatsapp_path,
    ...
    c.execute(
        'CREATE TABLE IF NOT EXISTS %s (contact TEXT, sender TEXT, datetime TEXT,'
    ...
    if digsby_path:
        ...
        if clean_table:
            c.execute(
                "DELETE FROM %s WHERE source = '%s'" % (sqlite_table, "Digsby"))
    ...
    if trillian_path:
        ...
        if clean_table:
            c.execute("DELETE FROM %s WHERE source = '%s'" % (sqlite_table,
                                                              "Trillian"))
    ...
    if pidgin_path:
        ...
        if clean_table:
            c.execute(
                "DELETE FROM %s WHERE source = '%s'" % (sqlite_table, "Pidgin"))
    ...
    if viber_path:
        ...
        if clean_table:
            c.execute(
                "DELETE FROM %s WHERE source = '%s'" % (sqlite_table, "Viber"))
    ...
    for gr in grouper(5000, lines(messages, self_name)):
        conn.executemany(
            "INSERT INTO %s (contact, sender, datetime, source,"
